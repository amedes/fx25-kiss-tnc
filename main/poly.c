...
void poly_copy(poly_t *to, poly_t *from)
{
    ...
    for (i = 0; i <= from->degree; i++) to->coeff[i] = from->coeff[i];
...
void poly_clear(poly_t *poly)
{
    ...
    for (i = 0; i < POLY_SIZE; i++) poly->coeff[i] = 0;
...
*/
static void poly_addsub(poly_t *p1, poly_t *p2, poly_t *result, gf_t (*gf_addsub)(gf_t x, gf_t y))
{
    int i;
    ...
    while ((i <= p1->degree) || (i <= p2->degree)) {
        result->coeff[i] = (*gf_addsub)((i <= p1->degree) ? p1->coeff[i] : 0, (i <= p2->degree) ? p2->coeff[i] : 0);
